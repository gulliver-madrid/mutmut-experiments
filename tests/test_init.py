...
def check_mutants_stub(**kwargs: Any) -> None:
    def run_mutation_stub(*_: Any, **_kwargs: Any) -> Literal["ok_killed"]:
        sleep(0.15)
        return OK_KILLED
...
class ConfigFlagsStub:
    parallelize = False


class ConfigStub:
    hash_of_tests = None
    flags = ConfigFlagsStub()
...
def test_run_mutation_tests_thread_synchronization(monkeypatch: Any) -> None:
    ...
    mutation_tests_runner.run_mutation_tests(config_stub, progress_mock, {})
